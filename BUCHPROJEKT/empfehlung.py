...
def find_similar_books(title, df, stopwords, top_n=5):
    ...
    tfidf = TfidfVectorizer(
        stop_words=combined_stopwords, ngram_range=(1, 2), max_df=0.7, min_df=3
    )
...
def show():
    ...
    selected_title = st.selectbox(
        "Wähle ein Buch aus:", sorted(books["title"].unique())
    )
    ...
    if selected_title:
        ...
        recommendations = find_similar_books(
            title=selected_title, df=books, stopwords=combined_stopwords, top_n=5
        )
